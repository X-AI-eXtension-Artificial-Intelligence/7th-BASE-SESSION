...
## 트레이닝 파라메터 설정하기
lr = 1e-3 #학습률
batch_size = 4 # 한 번에 얼마나 풀지
num_epoch = 100 # 몇 번 반복할 지
...
class UNet(nn.Module):
    def __init__(self):
        ...
        # Contracting path (왼쪽 부분 -> 사진을 줄이면서 정보 뽑아내는)
        # 두 번 3 * 3 (성능 향상을 위한 2번)
        self.enc1_1 = CBR2d(in_channels=1, out_channels=64)
        self.enc1_2 = CBR2d(in_channels=64, out_channels=64)
        # 크기 줄이기
        self.pool1 = nn.MaxPool2d(kernel_size=2)
        # 똑같은 방식으로 점점 정보 양 늘리고 사진 크기는 줄이고 
        # -> 전체적인 형태를 보고 픽셀이 아닌 의미를 찾기 위한 과정
        ...
        # Expansive path(오른쪽 - 이미지 복원)
        # 이미지 채널 줄이기
        self.dec5_1 = CBR2d(in_channels=1024, out_channels=512)
        # 사진 크기 키우기
        self.unpool4 = nn.ConvTranspose2d(in_channels=512, out_channels=512,
                                          kernel_size=2, stride=2, padding=0, bias=True)
        # 선명한 사진(왼쪽에서 저장해두었던)걸 이 512장에 연결하여 다시 그림
