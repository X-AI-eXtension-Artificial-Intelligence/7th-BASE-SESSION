"""
YOLOv1의 loss function을 정의하는 파일입니다.
bounding box 좌표 변환, IoU 계산, localization/objectness/classification loss 계산이 포함됩니다.
"""
...
# 중심점 형식의 bounding box를 corner 형식으로 바꾸는 함수
...
def iou(bboxes1_coords: th.Tensor, bboxes2_coords: th.Tensor) -> th.Tensor:
 ...
 # 겹치지 않는 경우 음수가 되지 않도록 clamp(min=0)를 적용
 ...
 # 0으로 나누는 문제를 막기 위해 작은 값을 더해줍니
    return intersection / (union + 1e-6)


# YOLOv1 논문의 loss를 PyTorch Module로 구현한 클래스
class YOLO_Loss(nn.Module):
 ...
 # 각 bounding box 예측값의 위치 index를 buffer로 저장 학습 파라미터는 아닙니
 ...
 # mini-batch 크기
        n = y_pred.shape[0]
 # 해당 grid cell에 객체가 존재하면 1, 아니면 0
 ...
 # 각 예측 box와 정답 box 사이의 IoU를 계산
        iou_scores = iou(gt_bboxes_coords_corners, pred_bboxes_coords_corners)
 # B개 box 중 정답과 가장 IoU가 높은 box를 responsible box로 선택
