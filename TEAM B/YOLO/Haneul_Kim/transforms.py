"""
YOLOv1 학습에 필요한 이미지/좌표 변환을 정의하는 파일입니다.
Resize, random scale/translate, color jitter, horizontal flip,
YOLO target tensor 변환 등이 포함됩니다.
"""
...
# 이미지 크기를 바꾸고 bounding box 좌표도 같은 비율로 조정하는 transform
class Resize:
    """
 ...
 # x 좌표는 width 변화 비율에 맞춰 스케일링
        target[:, [1, 3]] *= self.d / w
 # y 좌표는 height 변화 비율에 맞춰 스케일링
        target[:, [2, 4]] *= self.d / h
...
# resize/zoom in/zoom out을 랜덤하게 적용해 데이터 증강을 수행하는 transform
class RandomScaleTranslate:
 ...
 # 변환 후 너무 작아진 bounding box는 학습에 방해될 수 있어 제거
...
# 이미지의 hue/saturation/exposure를 랜덤하게 바꾸는 색상 증강 transform
...
# 이미지를 좌우 반전하고 bounding box 좌표도 함께 뒤집는 transform
...
# 일반 bounding box annotation을 YOLOv1 target tensor 형태로 변환하는 transform
class ToYOLOTensor:
    """
...
# PIL 이미지를 PyTorch tensor로 변환하고 정규화하는 transform
class ImgToTensor:
    """
