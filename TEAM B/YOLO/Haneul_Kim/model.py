"""
YOLOv1 모델 구조를 정의하는 파일입니다.
Convolution backbone, detection head, classification pretraining head,
그리고 YOLO 논문에 등장하는 locally connected layer를 구현합니다.
"""
...
# Convolution과 비슷하지만 위치마다 다른 weight를 쓰는 locally connected layer
class LocallyConnected2d(nn.Module):
 ...
 # 위치별로 다른 kernel weight를 학습 파라미터로 둡니
    ...
    def forward(self, x: th.Tensor) -> th.Tensor:
        ...
        """
 # 입력 feature map 가장자리에 padding을 추가
        x = F.pad(x, (self.padding,) * 4)
 # convolution처럼 sliding window patch를 추출
...
# YOLOv1의 convolution block들을 config 기반으로 만드는 클래스
...
# YOLOv1 전체 모델 클래스 mode에 따라 classification/detection 구조가 달라집니
class YOLOv1(nn.Module):
    ...
    paper's figure.
    """
 # 논문 구조에 맞춘 backbone convolution layer 설정
 ...
 # detection task에서 추가로 사용하는 convolution layer 설정
