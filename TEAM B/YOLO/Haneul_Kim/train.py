"""
PASCAL VOC 데이터셋으로 YOLOv1 detection 모델을 학습하는 파일입니다.
데이터 증강, YOLO loss, learning rate scheduler, checkpoint 저장 로직을 포함합니다.
"""
...
# YOLO 학습에 맞춘 custom learning rate scheduler
class MultiStepScaleLR:
    ...
    def step(self) -> None:
        ...
        """
 # weight update가 몇 번 이루어졌는지 batch counter를 증가
        self.batch += 1
 # 초반 burn-in 구간에서는 learning rate를 서서히 증가
...
def train_epoch(train_loader: DataLoader,
 ...
 # 모델을 학습 모드로 전환
    ...
    for x, y_gt in train_loader:
        ...
        y_pred = model(x)
 # subdivision을 사용하므로 loss를 나눠 gradient accumulation을 수행
        loss = criterion(y_pred, y_gt) / SUBDIVISIONS
 # loss를 기준으로 역전파를 수행해 gradient를 계산
        loss.backward()

 # 지정한 subdivision 수만큼 gradient가 누적되면 실제 optimizer step을 수행
        if mini_batch == SUBDIVISIONS:
 # 계산된 gradient를 이용해 모델 파라미터를 업데이트
        optimizer.step()
 # 이전 batch에서 누적된 gradient를 초기화
        optimizer.zero_grad()
            scheduler.step()
            mini_batch = 0
...
# validation/test set으로 loss를 계산하는 함수
...
# 학습에 필요한 모델, 데이터셋, optimizer, loss 등을 준비하는 함수
def setup_train():
    """
