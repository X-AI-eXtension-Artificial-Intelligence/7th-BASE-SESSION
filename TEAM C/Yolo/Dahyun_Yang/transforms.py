...
class RandomScaleTranslate:
    ...
    2. zoom out 후 resize
    3. zoom in 후 resize
    = 이미지가 확대/축소/이동되면 bbox도 같이 바뀐다는 것
    = 너무 작아진 bbox는 제거
    """
    def __init__(self,
                 output_size: int,
...
class RandomColorJitter:
    """
    색상을 랜덤하게 변경하고, bbox 좌표는 변하게 하지 않음
    padding(zoom out) 부분은 색이 변하지 않도록 mask를 사용함
    """
...
class ToYOLOTensor:
    """
    가장 중요한 부분
    이미지를 tensor로 바꾸고, bbox annotation을 YOLO 학습용 target 형식

    뒤의 YOLO loss 에서 쓰는 y_gt 형태를 여기서 생성함
    1. 입력 target
            [N, 5]
...
class ImgToTensor:
    """
       이미지만 tensor로 바꾸고 normalize
       bbox target은 그대로 두고, 주로 평가 부분에서 사용하게 됨
    """
