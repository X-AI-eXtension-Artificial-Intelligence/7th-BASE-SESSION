...
class AttnDecoder(nn.Module):
    def __init__(self, hidden_size, output_size):
        ...
        # 2*hidden_size인 이유:
        # embedded(hidden_size) + context(hidden_size) 를 concat해서 넣기 때문
    ...
    def forward_step(self, input, hidden, encoder_outputs, input_mask):
        ...
        embedded = self.embedding(input)
        #(이번 예측 단어 + 인코더에서 집중할 부분)을 동시에 고려
...
class EncoderDecoder(nn.Module):
    def __init__(self, hidden_size, input_vocab_size, output_vocab_size):
        ...
        #이 부분이 왜 주석처리되어있냐면 Attention이 있는 디코더와 그렇지않은 일반 디코더로
        #나중에 실험 결과 분석할때 비교해보도록 선택적 주석처리
